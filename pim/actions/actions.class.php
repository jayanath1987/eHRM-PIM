...
class pimActions extends sfActions {
    ...
    public function executeEmployeeList(sfWebRequest $request) { 
        try {
            ...
            $result = $employeeService->getEmployeeList($this->searchMode, $this->searchValue, $this->userCulture, $request->getParameter('page'), $this->sort, $this->order, $Active="1");
...
//            die(print_r($this->today));
            ...
            $result = $employeeService->searchEmployee($this->searchMode, $this->searchValue, $this->userCulture, $request->getParameter('page'), $this->sort, $this->order, $this->type, $this->method, $this->reason, $this->att, $this->payroll, $this->payRolltype, $this->locationWise, $this->startDate, $this->endDate, $this->empdef);
...
//        $sql.= " LIMIT {$datastartlimit}, {$dataperpage}";
    ...
    public function executeJpagination(sfWebRequest $request) {
        ...
        if($active != "" ){
            $sqlrow.= " where e.emp_active_hrm_flg = '{$active}' ";
            //$sqlrow.= " AND e.emp_active_hrm_flg = '0' ";
        }
        
        if ($searchmode != 'all' && $searchvalue != '') {
            //$sql.= " where {$searchColumn}  LIKE "."'%".trim($searchvalue)."%' ";
            $sqlrow.= " AND {$searchColumn}  LIKE '%".trim($searchvalue)."%' ";
        }

        if($user->def_level!= 1){
                    $sqlrow.= " AND e.emp_number = {$user->emp_number}";
        } 
        ...
        if($active != NULL ){
            $sql.= " where e.emp_active_hrm_flg = '{$active}' ";
        }
        
        if ($searchmode != 'all' && $searchvalue != '') {
            //$sql.= " where {$searchColumn}  LIKE "."'%".trim($searchvalue)."%' ";
            $sql.= " AND {$searchColumn}  LIKE '%".trim($searchvalue)."%' ";
        }

         if($user->def_level!= 1){
                    $sql.= "AND  e.emp_number = {$user->emp_number}";
        } 
        ...
        if (strlen($_SESSION['PIM_EMPID'])) {
            ...
            try {
                ...
                if ($request->isMethod('post')) { 
                    ...
                    if (strlen($request->getParameter('optSubject'))) {
                        $EMPHead->setEduh_flg(trim($request->getParameter('optSubject')));
                    } else {
                        $EMPHead->setEduh_flg(null);
                    }
                    if (strlen($request->getParameter('textDesc'))) {
                        $EMPHead->setEduh_descreption(trim($request->getParameter('textDesc')));
                    } else {
                        $EMPHead->setEduh_descreption(null);
                    }
...
//die;
    ...
    public function executeInactiveEmployeeList(sfWebRequest $request) { 
        try {
            $this->getUser()->setCulture($_SESSION['language']);
            $this->userCulture = $this->getUser()->getCulture();
            $employeeService = new EmployeeService();
            ...
            $result = $employeeService->getEmployeeList($this->searchMode, $this->searchValue, $this->userCulture, $request->getParameter('page'), $this->sort, $this->order, $Active='0');

...
<script type="text/javascript">
 <?php $sysconfig = new sysConf(); 
 //$sysconfig->rowLimit;
 
 ?>
    var pagelimit= "<?php echo $sysconfig->rowLimit; ?>";
    var eend=$("#PageRows").val();
    var lastpage=parseInt(eend/pagelimit);
    var remain=parseInt(eend%pagelimit);
    var sort;
    var childdiv;
    if(remain > 0){
        lastpage+=1;
 ...
 function showPage(page){  
     var dispaly;
     if(page!=1){
        var end= $("#PageRows").val();
        var page1=parseInt(end/pagelimit);
        dispaly=lastpage;        
     }else{ 
         dispaly=1;
     }
     
     var pager = new Pager('results', pagelimit); 
     pager.init();
     ...
     //pager.showPage(10);
        ...
        var childdiv="";
            ...
            data: { datapage: (page1) , searchmode: $('#cmbSearchMode').val() , searchvalue: $('#txtSearchValue').val(), sort:sort, active : '0'},
            dataType: "json",
     ...
     var rows = $("#PageRows").val();
        if(rows!=0){
        var records = (rows - 1); 
        this.pages = Math.ceil(records / pagelimit);
        this.inited = true;
        }
        var pager = new Pager('results', pagelimit); 
        if($("#PageRows").val()!=null){ 
        var element = document.getElementById('pageNavPosition');
        var pos=this.pages-5;
        var end=this.pages;
        var pagerName='pager';
        //var pagerName=pager.pagerName;
        var page=end-1;
        var pagerHtml = "";
        pagerHtml += '<span onclick="showPage(1);" class="pg-normal"> <?php echo __("First") ?> </span> <span onclick="pager.prev();" class="pg-normal"> <?php echo __("Prev") ?> </span> | ';
    	//var pagerHtml = '<span onclick="' + pagerName + '.prev();pager.prev()" class="pg-normal"> &#171 <?php echo __("Prev") ?> </span> | ';
        ...
        var start=0;
        for (var pagee = pos; pagee <= end; pagee++){ 
            //for (var page = 1; page <= this.pages; page++){ 
            pagerHtml += '<span id="pg' + pagee +'"';
            if(start != page){
            pagerHtml += 'class="pg-normal"';
            }else{ 
            pagerHtml += 'class="pg-selected"';    
            }
            pagerHtml += 'onclick="pager.showPage('+pagee+');">' + pagee + '</span> | ';
            start=pagee;
            } 
       // pagerHtml += '<span onclick="'+pagerName+'.next();" class="pg-normal"> <?php echo __("Next")?> &#187;</span>'; 
         pagerHtml += '<span onclick="pager.showPage('+lastpage+');" class="pg-normal"> <?php echo __("Last")?> </span>';
 ...
 function Pager(tableName, itemsPerPage) {
    ...
    this.showPage = function(pageNumber) { //alert(pageNumber);
...
//         }
        ...
        childdiv="";
            ...
            async:false,
            url: "<?php echo url_for('pim/Jpagination') ?>",
            data: { datapage: (this.currentPage-1) , searchmode: $('#cmbSearchMode').val() , searchvalue: $('#txtSearchValue').val(), sort:sort , active : '0'},
            dataType: "json",
            success: function(data){
                    var row = 0; 
                    $.each(data, function(key, value) {
    ...
    this.showPageNav = function(pagerName, positionId) { 
        ...
        if($("#PageRows").val()!=null){
    	//alert(this.currentPage);
        var pagerHtml = '<span onclick="showPage(1);" class="pg-normal"> <?php echo __("First") ?> </span> <span onclick="' + pagerName + '.prev();" class="pg-normal"> <?php echo __("Prev") ?> </span> | ';
       ...
       // pagerHtml += '<span onclick="'+pagerName+'.next();" class="pg-normal"> <?php echo __("Next")?> &#187;</span>'; 
         pagerHtml += '<span onclick="'+pagerName+'.next();" class="pg-normal"> <?php echo __("Next")?> </span><span onclick="showPage('+lastpage+');" class="pg-normal"> <?php echo __("Last")?> </span>';
        ...
        element.innerHTML = pagerHtml;
         ...
         pager.showPage(this.currentPage);
    }
}
